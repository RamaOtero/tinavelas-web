...
export default function Cart() {
  ...
  const handleWhatsappCheckout = () => {
    ...
    msg += `*MI CARRITO:*\n`;
    items.forEach(item => {
      msg += `- ${item.quantity}x ${item.name} ${item.scent ? `(Aroma: ${item.scent}) ` : ''}${item.lid ? `[Tapa: ${item.lid}] ` : ''}${item.reedType ? `[Varillas: ${item.reedType}] ` : ''}${item.hasLabel !== undefined ? (item.hasLabel ? '[C/ Etiqueta] ' : '[Sin Etiqueta] ') : ''}- ${formatPrice(item.price * item.quantity)}\n`;
    });
    ...
    try {
      const payload = {
        items: items.map(i => ({
          ...
          quantity: i.quantity,
          name: `${i.name}${i.scent ? ` (${i.scent})` : ''}${i.lid ? ` [Tapa: ${i.lid}]` : ''}${i.reedType ? ` [Varillas: ${i.reedType}]` : ''}${i.hasLabel !== undefined ? (i.hasLabel ? ' [Con Etiqueta]' : ' [Sin Etiqueta]') : ''}`, 
          image: null
        })),
        customer: { name, phone },
                      ...
                      <div className="flex-1 flex flex-col h-full justify-between py-1">
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1 mb-2">
                            ...
                            {item.reedType && (
                              <p className="text-[9px] font-sans tracking-widest text-text-dark/60 uppercase border border-text-dark/20 px-1.5 py-0.5 rounded-sm bg-text-dark/5">{item.reedType}</p>
                            )}
